...
//#   include "readFluxScheme.H"
    ...
    Info<< "\nStarting time loop\n" << endl;
Info<<"se consideran"<<Y.size()<<"Especies"<<endl;
...
//        surfaceVectorField U_pos("U_pos", rhoU_pos/rho_pos);
 ...
 surfaceVectorField rhoU_neg("rhoU_neg", U_neg*rho_neg);
  ...
  surfaceScalarField R_pos =
            fvc::interpolate(R, pos, "reconstruct(T)");
        surfaceScalarField R_neg =
            fvc::interpolate(R, neg, "reconstruct(T)");
     ...
     surfaceScalarField T_pos =
            fvc::interpolate(T, pos, "reconstruct(TT)");
        surfaceScalarField T_neg =
            fvc::interpolate(T, neg, "reconstruct(TT)");
...
*/
 ...
 surfaceScalarField cSf_pos
        (
            "cSf_pos",
            sqrt(Gamma_pos*R_pos*T_pos)*mesh.magSf()
        );

 surfaceScalarField cSf_neg
        (
            "cSf_pos",
            sqrt(Gamma_neg*R_neg*T_neg)*mesh.magSf()
        );
...
"phiT",
aphiv_pos*T_pos + aphiv_neg*T_neg
);
Info<<" phiT" <<phiT.dimensions()<<endl; 
/*
surfaceScalarField  phiT
(
    IOobject
